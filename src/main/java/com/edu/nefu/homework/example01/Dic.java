...
public class Dic {
    ...
    public void trans(String english) {
        ...
        for (int i = 0; i < size; i++) {
            if (dicItems[i].getEnglish().equalsIgnoreCase(english)) {//判断传进来的单词是否在词库中存在
    ...
    public void add(String english, String chinese) {
        ...
        int flag = 0;//标记词库中是否有这个单词，默认没有
        for (int i = 0; i < size; i++) {
            if (this.dicItems[i].getEnglish().equalsIgnoreCase(english)) {//判断传进来的单词是否在词库中存在
                System.out.println(english+" 词库中已有该单词");
                flag=1;
            }
        }
        if (flag==0){
            this.dicItems[size] = new DicItem(english,chinese);//单词添加
            size++;
        }
